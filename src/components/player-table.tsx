            ...
            <tr key={p.id}>
              <td>
                <Link className="pilot-name" href={`/players/${p.slug}`}>
                  ...
                  <strong>{p.name}</strong>{(p.detail as any)?.country && resolveCountryCode((p.detail as any).country) && <span className="table-flag" role="img" aria-label={`Bandera de ${(p.detail as any).country}`} style={{ backgroundImage: `url(https://flagcdn.com/w40/${resolveCountryCode((p.detail as any).country)}.png)` }} />}
                </Link>
              </td>
              <td>
