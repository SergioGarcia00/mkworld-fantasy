-- El cierre solo se procesa si la tienda activa corresponde al día actual.
-- Evita que el cron regenere la tienda siguiente cada minuto tras las 23:00.
create or replace function public.admin_first_week_tick()
returns integer language plpgsql security definer set search_path='' as $$
declare cfg public.app_config; local_now timestamp; today date; current_day date; next_day date; generated integer:=0; market_close time;
begin
  ...
  if current_day<today then perform public.admin_first_week_settle(current_day); generated:=public.admin_first_week_shop(today); end if;
  if local_now::time>=market_close
     and (select first_week_market_date from public.app_config where id=true)=today
     and (select first_week_market_open from public.app_config where id=true) then
    perform public.admin_first_week_settle(today);
    next_day:=today+1;
    if next_day<=date '2026-09-20' then generated:=public.admin_first_week_shop(next_day); end if;
  end if;
  if today=date '2026-09-20' and local_now::time>=time '19:00' then update public.app_config set test_lineup_open=false where id=true; end if;
  return generated;
end $$;
