-- A player can be sold again after being re-acquired; sales must not reuse a
-- permanent idempotency key tied only to the player.
...
begin
  ...
  select * into strict team_row from public.fantasy_teams
    where id = target_fantasy_team and user_id = auth.uid() for update;
  select * into strict roster_row from public.fantasy_roster_players
    where fantasy_team_id = team_row.id and player_id = target_player for update;
  ...
  values
    (team_row.id, target_player, 'PILOT_MARKET_SALE', sale_price, before_balance,
     ...
     jsonb_build_object('marketValue', player_value, 'sellPercentage', config_row.market_sell_percentage, 'salePrice', sale_price));
end $$;
